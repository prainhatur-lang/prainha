/* concilia inspector firebird - bundle */
"use strict";
...
function queryOne(cfg, sql, params = []) {
  const opts = {
  ...
  return new Promise((resolveP, rejectP) => {
      ...
      import_node_firebird.default.attach(opts, (err, db) => {
        ...
        try {
          db.query(sql, params, (e, rows) => {
            finish(() => e ? rejectP(e) : resolveP(rows ?? []));
            try {
              db.detach(() => {
              });
            } catch {
    ...
    setTimeout(() => finish(() => rejectP(new Error("timeout 15s"))), 15e3);
  });
}
...
function parseArgs(argv) {
  const FLAGS_COM_VALOR = /* @__PURE__ */ new Set([
    "host",
    "port",
    "path",
    "database",
...
async function main() {
  ...
  const outputPath = flags.output || (0, import_node_path2.resolve)(process.cwd(), "firebird-inspect.json");
  let cfg;
  try {
    cfg = loadConfig();
  } catch (e) {
    ...
    cfg = {
      ...
      firebird: {
        ...
        database: flags.path || flags.database || "",
        user: flags.user || "SYSDBA",
        password: flags.password || "masterkey"
      },
      intervalSeconds: 900,
      batchSize: 500,
  ...
  if (flags.host) cfg.firebird.host = flags.host;
  if (flags.port) cfg.firebird.port = Number(flags.port);
  if (flags.path || flags.database) {
    cfg.firebird.database = flags.path || flags.database;
  }
  if (flags.user) cfg.firebird.user = flags.user;
  if (flags.password) cfg.firebird.password = flags.password;
  console.log("configura\xE7\xE3o:");
  ...
  console.log("\nlistando tabelas de usu\xE1rio...");
  const tabelasRaw = await queryOne(
    cfg,
    ...
    try {
      ...
      if (dump.colunas.length > 0) {
        try {
          ...
          dump.erro = `count falhou: ${e.message}`;
        }
        if (!noSamples && dump.totalLinhas && dump.totalLinhas > 0) {
          try {
            const sample = await queryOne(cfg, `SELECT FIRST 3 * FROM "${nome}"`);
            dump.amostra = sample.map((r) => normalizar(r));
          } catch (e) {
            const msg = `amostra falhou: ${e.message}`;
  ...
  (0, import_node_fs2.writeFileSync)(outputPath, JSON.stringify(out, null, 2), "utf8");
  console.log(`
...
function tipoFirebird(tipo, subtipo, tamanho, escala, precisao) {
  const s = subtipo ?? 0;
  switch (tipo) {
    case 7:
      return "SMALLINT";
    ...
    case 16:
      return escala && escala < 0 && precisao ? `NUMERIC(${precisao},${-escala})` : "BIGINT";
    case 23:
      return "BOOLEAN";
    case 27:
