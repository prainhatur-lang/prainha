...
var require_database = __commonJS({
  "../node_modules/.pnpm/node-firebird@2.0.2/node_modules/node-firebird/lib/wire/database.js"(exports2, module2) {
    ...
    var Database = class extends Events.EventEmitter {
      ...
      newStatement(query2, callback) {
        this.startTransaction(function(err, transaction) {
          if (err) {
            callback(err);
            return;
          }
          transaction.newStatement(query2, function(err2, statement) {
            if (err2) {
              callback(err2);
              return;
            }
      ...
      execute(query2, params, callback, custom2) {
        ...
        self2.connection.startTransaction(function(err, transaction) {
          ...
          transaction.execute(query2, params, function(err2, result, meta, isSelect) {
            if (err2) {
      ...
      sequentially(query2, params, on, callback, asArray) {
        if (params instanceof Function) {
          asArray = callback;
          callback = on;
          on = params;
        ...
        self2.execute(query2, params, callback, { asObject: !asArray, asStream: true, on: _on });
        return self2;
      }
      query(query2, params, callback) {
        if (params instanceof Function) {
          callback = params;
          params = void 0;
        }
        var self2 = this;
        self2.execute(query2, params, callback, { asObject: true, asStream: callback === void 0 || callback === null });
        return self2;
      }
      drop(callback) {
  ...
  "../node_modules/.pnpm/node-firebird@2.0.2/node_modules/node-firebird/lib/wire/transaction.js"(exports2, module2) {
    ...
    var Transaction = class {
      ...
      newStatement(query2, callback) {
        ...
        var self2 = this;
        var query_cache = cnx.getCachedQuery(query2);
        if (query_cache) {
          callback(null, query_cache);
        } else {
          cnx.prepare(self2, query2, false, callback);
        }
      }
      execute(query2, params, callback, custom2) {
        if (params instanceof Function) {
        ...
        var self2 = this;
        this.newStatement(query2, function(err, statement) {
          if (err) {
            doError(err, callback);
            return;
          }
      ...
      query(query2, params, callback) {
        ...
        if (callback === void 0)
          callback = noop;
        this.execute(query2, params, callback, { asObject: true, asStream: callback === void 0 || callback === null });
      }
      commit(callback) {
...
var require_connection = __commonJS({
  "../node_modules/.pnpm/node-firebird@2.0.2/node_modules/node-firebird/lib/wire/connection.js"(exports2, module2) {
    ...
    var Connection = class _Connection {
      ...
      _setcachedquery(query2, statement) {
        if (this._cache_query) {
          if (this._max_cached_query === -1 || this._max_cached_query > Object.keys(this._cache_query).length) {
            this._cache_query[query2] = statement;
          }
        }
      }
      getCachedQuery(query2) {
        return this._cache_query ? this._cache_query[query2] : null;
      }
      _bind_events(host, port, callback) {
        var self2 = this;
        self2._socket.on("close", function() {
      ...
      allocateAndPrepareStatement(transaction, query2, plan, callback) {
        var self2 = this;
        var mainCallback = function(err, ret) {
          if (!err) {
            mainCallback.response.handle = ret.handle;
            describe(ret.buffer, mainCallback.response);
            mainCallback.response.query = query2;
            self2.db.emit("query", query2);
            ret = mainCallback.response;
            self2._setcachedquery(query2, ret);
          }
          if (callback)
            callback(err, ret);
        };
        ...
        msg.addInt(65535);
        msg.addInt(3);
        msg.addString(query2, Const.DEFAULT_ENCODING);
        msg.addBlr(blr);
        msg.addInt(65535);
        mainCallback.lazy_count += 1;
      ...
      prepare(transaction, query2, plan, callback) {
        var self2 = this;
        if (this.accept.protocolMinimumType === Const.ptype_lazy_send) {
          self2.allocateAndPrepareStatement(transaction, query2, plan, callback);
        } else {
          self2.allocateStatement(function(err, statement) {
            ...
            self2.prepareStatement(transaction, statement, query2, plan, callback);
          });
        }
      }
      prepareStatement(transaction, statement, query2, plan, callback) {
        if (this._isClosed)
          return this.throwClosed(callback);
        var msg = this._msg;
        var blr = this._blr;
...
function executarWrites(cfg, queries) {
  const opts = {
    host: cfg.firebird.host,
    port: cfg.firebird.port,
    database: cfg.firebird.database,
  ...
  const inner = new Promise((resolve5, reject) => {
    ...
    import_node_firebird.default.attach(opts, (err, db) => {
      ...
      db.transaction(import_node_firebird.default.ISOLATION_READ_COMMITTED, (txErr, tx) => {
        if (txErr) {
          cleanup();
        ...
        const next = (i) => {
          ...
          const q = queries[i];
          tx.query(q.sql, q.params, (qErr, result) => {
            if (qErr) {
              tx.rollback(() => cleanup());
...
function executarWrite(cfg, sql, params) {
  const opts = {
    host: cfg.firebird.host,
    port: cfg.firebird.port,
    database: cfg.firebird.database,
  ...
  const inner = new Promise((resolve5, reject) => {
    ...
    import_node_firebird.default.attach(opts, (err, db) => {
      ...
      db.transaction(import_node_firebird.default.ISOLATION_READ_COMMITTED, (txErr, tx) => {
        ...
        tx.query(sql, params, (qErr, result) => {
          if (qErr) {
            tx.rollback(() => cleanup());
            return safeReject(qErr);
          }
...
async function buscarFornecedoresJanela(cfg, desdeCodigoNaJanela, limite) {
  const sql = `
...
async function buscarClientesPorTabela(cfg, tabela, desdeCodigo, limite) {
  const rows = await selectFlexivel(
    cfg,
    tabela,
    [
  ...
  const sn = (v) => v === "S" ? true : v === "N" ? false : null;
  return rows.map((r) => ({
    codigoExterno: r.CODIGO,
...
async function baixarFiado(cfg, codigoCliente, observacao, valorBaixar) {
  const ultimo = await executarQuery(
    cfg,
  ...
  const credito = valorBaixar != null && valorBaixar > 0 ? Math.min(valorBaixar, saldoAnterior) : saldoAnterior;
  const saldoNovo = Math.round((saldoAnterior - credito) * 100) / 100;
  const obsLimpa = observacao.slice(0, 200);
  const sqlInsert = `
  ...
  const codigo = ins == null ? null : Array.isArray(ins) ? ins[0]?.CODIGO ?? null : ins.CODIGO ?? null;
  return { saldoAnterior, saldoNovo, codigo };
}
async function criarContaPagar(cfg, p) {
  const sql = `
  ...
  const params = [
    p.codigoCategoria,
    p.codigoFornecedor,
    /* @__PURE__ */ new Date(p.dataVencimento + "T00:00:00"),
    p.valor,
...
async function buscarPedidoAbertoPorMesa(cfg, numero) {
  const rows = await executarQuery(
    cfg,
...
async function abrirMesa(cfg, p) {
...
async function lancarBebidaNaComanda(cfg, p) {
  ...
  const itemRows = await executarQuery(
    cfg,
...
async function executarUpdate(cfg, tabela, campos, whereCodigo) {
...
async function criarContato(cfg, campos) {
  const fixos = {
    TIPO: "CF",
  ...
  const opcionais = {};
  for (const [k, v] of Object.entries(campos)) {
    if (v !== null && v !== void 0 && v !== "") opcionais[k] = v;
  }
  ...
  const r = await executarWrite(cfg, sql, params);
...
// src/drenador.ts
var import_node_firebird2 = __toESM(require_lib(), 1);
...
// src/drenador.ts
function attachFb(cfg) {
  return new Promise((resolve5, reject) => {
    import_node_firebird2.default.attach(
      {
        host: cfg.firebird.host,
        port: cfg.firebird.port,
        database: cfg.firebird.database,
      ...
      (e, db) => e ? reject(e) : resolve5(db)
    );
  });
}
function detachFb(db) {
...
function query(db, sql, params = []) {
  return new Promise((resolve5, reject) => {
    db.transaction(import_node_firebird2.default.ISOLATION_READ_COMMITTED, (e, tx) => {
      if (e) return reject(e);
      tx.query(sql, params, (e2, r) => {
        if (e2) {
          tx.rollback(() => reject(e2));
          return;
        }
        tx.commit((e3) => e3 ? reject(e3) : resolve5(r ?? []));
      });
    });
  });
}
function execWrite(db, sql) {
  return new Promise((resolve5, reject) => {
    db.transaction(import_node_firebird2.default.ISOLATION_READ_COMMITTED, (e, tx) => {
      if (e) return reject(e);
      tx.query(sql, [], (e2) => {
        ...
        tx.commit((e3) => e3 ? reject(e3) : resolve5());
      });
    });
  });
}
...
var PK_DEFAULT = "CODIGO";
function pkOf(tabela) {
  return PK_POR_TABELA[tabela] ?? PK_DEFAULT;
}
function escapeLiteral(v) {
  return "'" + v.replace(/'/g, "''") + "'";
}
async function lerFila(db, limite) {
...
var SELECT_ESPECIAL = {
  ITENSPEDIDO: (inList, pk) => `SELECT i.*,        COALESCE(i.CODIGOPRODUTO, pd.CODIGOPRODUTO) AS CODIGOPRODUTORESOLVIDO FROM ITENSPEDIDO i LEFT JOIN PRODUTODETALHE pd ON pd.CODIGO = i.CODIGOPRODUTODETALHE WHERE i.${pk} IN (${inList})`,
  ...
  PAGAMENTOS: (inList, pk) => `SELECT p.*,        TRIM(fp.DESCRICAO) AS FORMA FROM PAGAMENTOS p LEFT JOIN FORMASPAGAMENTO fp ON fp.CODIGO = p.CODIGOFORMAPAGAMENTO WHERE p.${pk} IN (${inList})`
};
async function buscarRegistros(db, tabela, chaves) {
  if (chaves.length === 0) return {};
  const pk = pkOf(tabela);
  const todasNumericas = chaves.every((c) => /^-?\d+$/.test(c));
  const inList = todasNumericas ? chaves.join(",") : chaves.map(escapeLiteral).join(",");
...
async function marcarProcessado(db, ids) {
...
async function marcarErro(db, ids, erro) {
  if (ids.length === 0) return;
...
async function enviarSync(cfg, registros) {
  ...
  try {
    ...
    clearTimeout(timer);
    const msg = e.name === "AbortError" ? `timeout 90s em POST /api/concilia/sync (payload ${body.length}B, ${registros.length} regs)` : e.message;
    throw new Error(msg);
  }
  clearTimeout(timer);
...
async function cicloDrenador(cfg) {
  ...
  try {
    ...
    for (let iter = 0; iter < MAX_ITER; iter++) {
      ...
      const porTabela = /* @__PURE__ */ new Map();
      ...
      for (const [tabela, items] of porTabela) {
        ...
        for (const d of deletes) idsOk.push(d.id);
      }
    ...
    try {
      ...
      log.warn("drenador: housekeeping falhou", { err: e.message });
    }
  } finally {
...
async function reportarComando(cfg, id, status, resultado) {
  const url = `${cfg.api.url.replace(/\/$/, "")}/api/agente/comandos`;
  await fetch(url, {
    method: "PATCH",
    headers: {
...
// src/cdc.ts
var import_node_firebird3 = __toESM(require_lib(), 1);
var TABELAS_CDC = [
  // [tabela, coluna_pk]
  // Catalogo (8)
  ["PRODUTOTIPO", "CODIGO"],
...
function attachFb2(cfg) {
  return new Promise((resolve5, reject) => {
    import_node_firebird3.default.attach(
      {
        host: cfg.firebird.host,
        port: cfg.firebird.port,
        database: cfg.firebird.database,
...
function exec(db, sql) {
  return new Promise((resolve5, reject) => {
    db.transaction(import_node_firebird3.default.ISOLATION_READ_COMMITTED, (e, tx) => {
      if (e) return reject(e);
      tx.query(sql, [], (e2, r) => {
        if (e2) {
          tx.rollback(() => reject(e2));
...
async function instalarCdc(cfg, dias = 365) {
  log.info("cdc: iniciando instalacao", { dias });
  const db = await attachFb2(cfg);
  const res = {
    estruturasCriadas: 0,
...
async function desinstalarCdc(cfg) {
  log.info("cdc: iniciando desinstalacao");
  const db = await attachFb2(cfg);
  const res = { triggersRemovidos: 0, estruturasRemovidas: 0 };
  try {
...
async function statusCdc(cfg) {
  ...
  try {
    ...
    const trigs = await exec(
      db,
      "SELECT count(*) AS N FROM RDB$TRIGGERS WHERE RDB$TRIGGER_NAME STARTING WITH 'TR_CONCILIA'"
    );
    ...
    return {
      ...
      exemploErros: exemplos.map((e) => ({
        ...
        erro: e.ULTIMO_ERRO ? e.ULTIMO_ERRO.trim() : null
      }))
    };
  } finally {
    await detachFb2(db);
...
async function pularTabelas(cfg, tabelas) {
  const db = await attachFb2(cfg);
  try {
...
async function cicloPdvRefetch(cfg) {
  ...
  try {
    ...
    log.warn("refetch fornecedores falhou", { err: e.message });
  }
}
async function autoUpdate(cfg, versao, comandoId) {
  ...
  try {
    ...
    if (buf.length < 1e5) throw new Error(`arquivo suspeito (${buf.length} bytes)`);
    const baseDir = (0, import_node_path4.dirname)(process.execPath);
    const alvo = (0, import_node_path4.resolve)(baseDir, "agente.cjs");
    log.info("auto_update: salvando", { alvo, bytes: buf.length });
    (0, import_node_fs5.writeFileSync)(alvo, buf);
...
async function cicloComandos(cfg) {
  let comandos;
  try {
    comandos = await buscarComandosPendentes(cfg);
  } catch (e) {
    log.warn("falha buscando comandos", { err: e.message });
    return;
  }
  ...
  for (const cmd of comandos) {
    try {
      ...
      if (cmd.tipo === "baixar_fiado") {
        const codCliente = cmd.payload.codigoCliente;
        const obs = cmd.payload.observacao ?? "Compensado em folha";
        const valorBaixar = cmd.payload.valorBaixar;
      ...
      if (cmd.tipo === "instalar_cdc") {
        const dias = cmd.payload.dias ?? 365;
      ...
      if (cmd.tipo === "pular_tabelas") {
        const tabelas = cmd.payload.tabelas ?? [];
      ...
      if (cmd.tipo === "reenviar_pagamentos") {
        ...
        if (!Number.isFinite(de) || !Number.isFinite(ate) || ate < de || ate - de > 5e3) {
        ...
        try {
          ...
          for (let i = 0; i < rows.length; i += 500) {
            const registros = rows.slice(i, i + 500).map((r) => ({
              tabela: "PAGAMENTOS",
              operacao: "U",
          ...
          await reportarComando(cfg, cmd.id, "erro", { msg: e.message });
        }
        continue;
      }
      if (cmd.tipo === "auto_update") {
        const versao = cmd.payload.versao;
      ...
      log.warn("comando falhou", { id: cmd.id, err: e.message });
      try {
        await reportarComando(cfg, cmd.id, "erro", { msg: e.message });
      } catch {
      }
...
async function loopComandos(cfg) {
  for (; ; ) {
    try {
      ...
      log.error("ciclo comandos falhou", { err: e.message });
...
async function main() {
  ...
  log.info("agente iniciado", {
    ...
    intervaloComandos: `${COMANDOS_INTERVAL_MS / 1e3}s`
  });
  const checkpoint = new Checkpoint(cfg.checkpointFile);
  loopComandos(cfg).catch((e) => log.error("loop comandos morreu", { err: e.message }));
  for (; ; ) {
    try {
    ...
    try {
      ...
      log.error("ciclo drenador falhou", { err: e.message });
      if (e.message?.includes("timeout")) {
