// POST /api/equipe/[filialId]/permissao — liga/desliga UMA permissão de um
// usuário do Consumer (idempotente na loja: repetir não duplica nem erra).
...
const Body = z.object({
  ...
  permissao: z.number().int().positive(),
  ligar: z.boolean(),
});
...
export async function POST(request: Request, { params }: { params: Promise<{ filialId: string }> }) {
  ...
  const r = await chamarLojaEquipe(filialId, '/permissao', { method: 'POST', body: parsed.data });
