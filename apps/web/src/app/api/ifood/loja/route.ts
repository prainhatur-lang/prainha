...
import { statusLoja, pausarLoja, retomarLoja, merchantsDoApp } from '@/lib/ifood-loja';
...
export async function GET(request: Request) {
  ...
  if (new URL(request.url).searchParams.get('merchants') === '1') {
    ...
    for (const c of todas) {
      ...
      try {
        ...
        grupos.push({ clientId: c.clientId, lojas: [], erro: (e as Error).message.slice(0, 240) });
      }
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 403 aqui é falta do módulo Merchant no app, não credencial errada.
    const semModulo = /→ 403/.test(msg);
    return NextResponse.json({
      error: semModulo
        ? 'este app ainda não tem o módulo Merchant liberado no Portal do Desenvolvedor — pausar/retomar seguem pelo iFood Gestor de Pedidos'
        : msg.slice(0, 300),
      semModulo,
