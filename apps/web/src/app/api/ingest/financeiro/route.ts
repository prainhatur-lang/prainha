...
const ClienteSchema = z.object({
  ...
  bloquearVendaAposLimite: z.boolean().nullable().optional(),
  arquivarFiado: z.boolean().nullable().optional(),
  // Cadastro completo — só vem do agente >= v1.2.0; ausente nas versões
  // antigas, e o upsert preserva o que já está gravado.
    ...
    const rows = clientes.map((c) => ({
      ...
      limiteCreditoContaCorrente:
        c.limiteCreditoContaCorrente !== null && c.limiteCreditoContaCorrente !== undefined
          ...
          : null,
      bloquearVendaAposLimite: c.bloquearVendaAposLimite ?? null,
      arquivarFiado: c.arquivarFiado,
      celular: c.celular?.slice(0, 30) ?? null,
      dataNascimento: c.dataNascimento ? c.dataNascimento.slice(0, 10) : null,
      endereco: c.endereco?.slice(0, 120) ?? null,
      numero: c.numero?.slice(0, 20) ?? null,
      complemento: c.complemento?.slice(0, 100) ?? null,
      bairro: c.bairro?.slice(0, 100) ?? null,
      cidade: c.cidade?.slice(0, 100) ?? null,
      uf: c.uf?.slice(0, 2) ?? null,
      cep: c.cep?.replace(/\D/g, '').slice(0, 10) || null,
      dataDelete: c.dataDelete ? new Date(c.dataDelete) : null,
      versaoReg: c.versaoReg,
      sincronizadoEm: new Date(),
    }));
      ...
      await db
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            bloquearVendaAposLimite: drizzleSql`COALESCE(excluded.bloquear_venda_apos_limite, ${schema.cliente.bloquearVendaAposLimite})`,
