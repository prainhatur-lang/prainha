...
export function BancoForm({ filiais }: { filiais: Filial[] }) {
  ...
  useEffect(() => {
    ...
    let cancelado = false;
    const qs = new URLSearchParams({ filialId, processo: 'BANCO', dataInicio, dataFim });
  ...
  async function rodar() {
    ...
    setError(null);
    try {
      const r = await fetch('/api/conciliacao/banco', {
        method: 'POST',
      ...
      router.push(`/conciliacao/banco?filialId=${filialId}`);
      router.refresh();
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-0.5 text-xs text-slate-500">Cruza Recebíveis × Extrato do banco.</p>
